...
def draw_lanes(img, left_fit, right_fit):
    ...
    cv2.polylines(color_img, np.int32([left]), isClosed=False, color=(255, 255, 255), thickness=15)
    cv2.polylines(color_img, np.int32([right]), isClosed=False, color=(255, 255, 255), thickness=15)
...
def vid_pipeline(img):
    ...
    cv2.putText(img, 'Curvatura da pista: {:.0f} m'.format(lane_curve), (450, 50), font, 0.7, (0, 255, 255), 1)
    cv2.putText(img, 'Deslocamento do Carro: {:.4f} m'.format(curverad[2]), (450, 80), font, 0.7, (0, 255, 255), 1)
    ...
    if (len(cars) > 0) or (len(bikes) > 0) or (len(buses) > 0) or (len(pedestrians) > 0):
        ...
        if len(bikes) > 0:
            for (x, y, w, h) in bikes:
                cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 215), 1)
        ...
        if len(pedestrians) > 0:
            for (a, b, c, d) in pedestrians:
                cv2.rectangle(img, (a, b), (a + c, b + d), (0, 255, 210), 1)
...
while True:
    ...
    if ret:
        ...
        cv2.putText(kp, f'Tempo de Processamento: {round((time.time() - start_time), 4)} Segundos', (450, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.rectangle(kp, (200, 535), (200 + 880, 535 + 60), (0, 0, 255), 1)
        cv2.putText(kp, 'Controle Visual', (530, 520), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # Draw arrow instead of green ball
        pt1 = (int(lane_center_pos), 570)
        pt2 = (int(lane_center_pos), 520)
        cv2.arrowedLine(kp, pt1, pt2, (0, 255, 0), 8, tipLength=0.3)

        offset = int(630 - lane_center_pos)
        steering_commands, side, PWM = calc_servo_pos(offset)
        cv2.putText(kp, f"OBS: {steering_commands} ", (450, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 1)
        print(steering_commands)
        cv2.putText(kp, f"Comandos: SIDE={side} PWM(DC)={PWM}% ", (450, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                    (0, 255, 255), 2)
